...
#%% Get Data
#%%% Web Scraping
# Tutorial:     https://www.youtube.com/watch?v=jniAyBPW7P8&list=PL3JVwFmb_BnSPEOC1E8a83Pk0czQOK10h
import os
...
edge_driver_path = os.path.join('C:/Users/49157/Documents/Python/SHK4FE-Digital-Twin/LiveTwin', 'msedgedriver.exe')
edge_service = Service(edge_driver_path)
edge_options = Options()
edge_options.add_argument(f'user-agent={user_agent}')

browser = webdriver.Edge(service = edge_service, options = edge_options)
browser.get('https://village-school.mondas.io/login/')
time.sleep(1)

browser.find_element(By.XPATH,".//*[@id='login_username']").send_keys("buehler")

browser.find_element(By.XPATH,".//*[@id='login_password']").send_keys("ad#bueh#village")
browser.find_element(By.CLASS_NAME,'ant-btn.ant-btn-primary').click() 
time.sleep(3)

#----------------------------------------------Change TimeRange----------------------------------------------

browser.get('https://village-school.mondas.io/admin/timeranges')
time.sleep(3)

# Timerange auf Live
button_xpath = '//*[@id="mondas-react-root"]/div/div/div[2]/div/div/div/div/div/div/div/div/table/tbody/tr[9]/td[4]/button/div'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(3)

# Bis Jetzt -> Bis Ende
button_xpath = '//*[@id="mondas-react-root"]/div/div/div[2]/div/div/div/div/div/div/div/div/table/tbody/tr[9]/td[5]/div/span[2]/div'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(3)
button_xpath = '/html/body/div/div/div/div/div[2]/div[1]/div/div/div[3]/div'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(3)

# Stunde -> Tag
button_xpath = '//*[@id="mondas-react-root"]/div/div/div[2]/div/div/div/div/div/div/div/div/table/tbody/tr[9]/td[5]/div/div/div[2]/div/span[2]'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(3)
button_xpath = '/html/body/div[2]/div/div/div/div[2]/div[1]/div/div/div[4]'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(3)

# Verschiebung um -1
button_xpath = '//*[@id="mondas-react-root"]/div/div/div[2]/div/div/div/div/div/div/div/div/table/tbody/tr[9]/td[5]/div/div/div[1]/div[1]/span[2]/span'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(3)

#----------------------------------------------Download----------------------------------------------
browser.get('https://village-school.mondas.io/download')
time.sleep(3)

button_xpath = '//*[@id="mondas-react-root"]/div/div/div/div[2]/div/div[2]/div/main/div/div/div[1]/button/span'
browser.find_element(By.XPATH, button_xpath).click()
time.sleep(20)
...
#%%% Move CSV to working Directory
import shutil
file_to_move = max(os.listdir('C:/Users/49157/Downloads'), key=lambda f: os.path.getmtime(os.path.join('C:/Users/49157/Downloads', f)))
source_directory = 'C:/Users/49157/Downloads'
destination_directory = os.path.dirname(os.path.abspath(__file__))  + '/Data'
shutil.move(os.path.join(source_directory, file_to_move), os.path.join(destination_directory, file_to_move))

#%%% Rename CSV
directory_path = os.path.dirname(os.path.abspath(__file__))  + '/Data'
current_file_name = max(os.listdir(os.path.dirname(os.path.abspath(__file__))  + '/Data'), key=lambda f: os.path.getmtime(os.path.join(os.path.dirname(os.path.abspath(__file__))  + '/Data', f)))
a = pd.read_csv(os.path.join(directory_path, current_file_name), index_col = 0, sep = ';', header = 1)
a.index = pd.to_datetime(a.index, format='%Y/%m/%d %H:%M:%S')
new_file_name = str(a.index[0].year) + '_' + str(a.index[0].strftime('%m')) + '_' + str(a.index[0].strftime('%d')) + '_' + '.csv'
...
def PVT(theta_mean_prev, PVT_theta_in, PVT_Vpkt, theta_e, BAT_SOC, G):
    ...
    E = PVT_Vpkt/3600*prop['Antifrogen']['rho']*prop['Antifrogen']['cp'] / (param['PVT']['A'] * param['PVT']['quantity']*2)
...
file = max(os.listdir(os.path.dirname(os.path.abspath(__file__))  + '/Data'), key=lambda f: os.path.getmtime(os.path.join(os.path.dirname(os.path.abspath(__file__))  + '/Data', f)))
...
#%% Wochenbilanz (Immer Montags)
if df.index[0].weekday() == 6:
    print('week')
    df = pd.DataFrame()                     # index als DateTimeObject
    for weeks in os.listdir(os.getcwd()+'\Data')[-7:]:
        print(weeks)
    ...
    ax31.text(5.45, 20.5,  r'$\bf{Gesamte\ Woche:}$' + '\n' + str(round(df['HM4_Qpkt'].sum()*0.25/1000,2)) + ' Wh gemessen'
              + '\n' + str(round(df['TABS_Qpkt'].sum()*0.25/1000,2)) + ' Wh simuliert' + '\n' +
              str(round((df['TABS_Qpkt'].sum()*0.25-df['HM4_Qpkt'].sum()*0.25)/(df['HM4_Qpkt'].sum()*0.25)*100,2)) + ' % Abweichung',
    ...
    ax41.text(5.4, 25.4,  r'$\bf{Gesamte\ Woche:}$' + '\n' + str(round(df['HM1_Qpkt'].sum()*0.25/1000,2)) + ' kWh gemessen'
              + '\n' + str(round(df['PVT_Qpkt'].sum()*0.25/1000,2)) + ' Wh simuliert' + '\n' +
              str(round((df['PVT_Qpkt'].sum()*0.25-df['HM1_Qpkt'].sum()*0.25)/(df['HM1_Qpkt'].sum()*0.25)*100,2)) + ' % Abweichung',
              fontsize = labelfont, bbox=dict(facecolor='white', edgecolor='black'))
