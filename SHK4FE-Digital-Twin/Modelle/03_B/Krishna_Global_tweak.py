...
param = {}
param['Position'] = {
    'lon' : 7.836136538382208,
    'lat' : 48.01003587039855,
    }
l = 4.55; b = 3.55 ; 
h = 2.42
param['B'] = {
    ...
    'transparent components': {
        ...
        'shading'    : [0,      0,      1,      0.6,      0,      0],           # -         1 for no shading, 0 for absolutely shadowed
        'g_tot'      : [0.4,    0.4,    0.4,    0.5,    0.4,    0],             # -         DIN 4108-2 8.3.2 (3) und DIN EN ISO 13790 Tabelle 7   Table G.1 — Typical values of total solar energy transmittance
        ...
        'A'          : [0,      0,      0,      3.4161, 0,      0],             # m²        Area of the component
        'direction'  : [41,     135,    225,    323,    270,    270],           # °         0° -> north, 90° -> east, 180° ->south, 270° -> west
        'tilt'       : [90,     90,     90,     90,     0,      180],           # °         0° horizontal top, 90° -> vertical, 180° -> horizontal down
        'U'          : [1.1]*6,                                                 # W/m²/K    Heat transfer coefficient determined according to ISO 6946
        },
    'opaque components': {
        # each column is one Component
        'shading'    : [0,      0,      1,      0.5,      0,    0],               # -         1 for no shading, 0 for absolutely shadowed
...
query = '''from(bucket: "Mosmix")
  |> range(start: 2023-08-07T00:00:00Z, stop: 2023-08-13T00:00:00Z)
  |> filter(fn: (r) => r["_measurement"] == "10803")
...
query = '''from(bucket: "heatguide101")
  |> range(start: 2023-08-07T00:00:00Z, stop: 2023-08-13T00:00:00Z)
   |> filter(fn: (r) => r["NodeID"] == "Node2")
