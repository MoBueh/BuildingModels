...
param['B'] = {
    ...
    'transparent components': {
        ...
        'shading'    : [0,      0,      0,      0.1,      0,      0],           # -         1 for no shading, 0 for absolutely shadowed
        ...
        'U'          : [1.0]*6,                                                 # W/m²/K    Heat transfer coefficient determined according to ISO 6946
        },
    'opaque components': {
        # each column is one Component
...
query = '''from(bucket: "Mosmix")
  |> range(start: 2023-08-21T00:00:00Z, stop: 2023-08-27T00:00:00Z)
...
query = '''from(bucket: "heatguide118")
  |> range(start: 2023-08-21T00:00:00Z, stop: 2023-08-27T00:00:00Z)
   |> filter(fn: (r) => r["NodeID"] == "Node35")
...
for i in df.index:
    ...
    if i.hour in [6, 9, 12, 18, 0] and i.minute == 0 and i.second == 0:
