...
# #%%
# param = {'A_f': 20.96,                                                          # m²    conditioned area
...
#         'R'    : 0.75,                                                          # Wärmeübergang Strahlung
#         }    
#     }

#%% Modell
def B5R1C(phi_int, theta_e, V_pkt, theta_sup, phi_HC, theta_m_prev, i_sol, shading, param):
...
# Parameter
    A_m = param['A_f']*param['Coe_Am']
    A_tot = 4.5 * param['A_f']
    c_m = param['Coe_Cm']*param['A_f']  # J/K
    # print(c_m)
    H_tr_w = sum([param['transparent']['U']*A for A in param['transparent']['A']])
    H_ve = 1200*V_pkt/3600         # Infiltration
    # H_ve = (1-param['ventilation']['etha_wrg'])*1200*V_pkt/60         # Für Wärmerückgewinnung
    if V_pkt == 0:
        H_ve = 0.00000001
    else:
        H_ve = 1*1200*V_pkt/3600
    H_tr_ms = 9.1*A_m
    H_op = sum([param['opaque']['U']*A for A in param['opaque']['A']])
...
# solar heat gain inside zone due to transparent components
    phi_sol_trans = []
    for shading, A, i in zip(shading, param['transparent']['A'], i_sol):
        phi_sol_trans.append(shading* param['transparent']['g_tot']*0.9*A*i)

# solar heat gain inside zone due to opaque components
    phi_sol_op = []
    for A, i in zip(param['opaque']['A'], i_sol):
        phi_sol_op.append(param['opaque']['R']*param['opaque']['U']*A*i)   
...
# Equation for Model regarding Appendix C
    ...
    theta_m_t = (theta_m_prev * (c_m/60-0.5 * (H_tr_3 +H_tr_em))+phi_mtot)/(c_m/60+0.5*(H_tr_3+H_tr_em))  # C.4
    # Output
    ...
    return theta_m, theta_s, theta_air, theta_operativ, phi_sol
