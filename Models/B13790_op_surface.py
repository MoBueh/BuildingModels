...
# Equation for Model regarding Appendix C
    ...
    phi_st = (1-A_m/A_tot-H_tr_w/9.1/A_tot)*(0.5 * phi_int + phi_sol) + phi_HC           # C.3
    phi_mtot = phi_m + H_tr_em * theta_e + H_tr_3 *(phi_st +H_tr_w * theta_e + H_tr_1 * ((phi_ia)/H_ve + theta_sup))/ H_tr_2   # C.5
    ...
    theta_s = (H_tr_ms * theta_m + phi_st + H_tr_w * theta_e + H_tr_1 *(theta_sup + (phi_ia)/H_ve))/(H_tr_ms + H_tr_w + H_tr_1)     # C.10
    theta_air = (H_tr_is * theta_s + H_ve * theta_sup + phi_ia)/(H_tr_is + H_ve)          # C.11
    theta_operativ = 0.3 * theta_air + 0.7 * theta_s                            # C.12
    return theta_m, theta_s, theta_air, theta_operativ

#%% Testing

    GB = building_op(phi_int = 1, 
                     theta_e = 20, 
                     V_pkt = 75, 
                     theta_sup = 20, 
                     phi_HC = 5000,
                     theta_m_prev = 20,
                     i_sol = [100,100,100,100,100],
                     shading = [1]*5,
                     param = param)
